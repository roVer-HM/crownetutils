...
from crownetutils.analysis.dpmm.hdf.dpmm_global_positon_provider import DpmmGlobalPosition
...
class CellEntropyValueError:
    ...
    def hdf_cell_entropy_measure(self) -> BaseHdfProvider:
        if self._hdf_cell_entropy_measure is None:
            self._hdf_cell_entropy_measure = BaseHdfProvider(self.hdf_path, group=self.g_error_cell_all)
    ...
    def hdf_cell_entropy_measure_rsd(self) -> BaseHdfProvider:
        if self._hdf_cell_entropy_measure_rsd is None:
            self._hdf_cell_entropy_measure_rsd = BaseHdfProvider(self.hdf_path, group=self.g_error_cell_rsd_all)
    ...
    def hdf_cell_entropy_measure_rsd_local(self) -> BaseHdfProvider:
        if self._hdf_cell_entropy_measure_rsd_local is None:
            self._hdf_cell_entropy_measure_rsd_local = BaseHdfProvider(self.hdf_path, group=self.g_error_cell_rsd_local)
    ...
    def load_glb(self, count_p: DpmmCount, time_slice: slice, builder:CellCountErrorBuilder) -> pd.DataFrame:
        _i = pd.IndexSlice
    ...
    def time_chunked_stream(self, count_p:DpmmCount, time_chunk_size, time_interval, builder:CellEntropyValueErrorBuilder):
        if time_chunk_size is None:
            time_slice = slice() # empty slice, no chunking load all data (can cause OOM!)
            ...
            while(start < time_interval[1]):
                ...
                print(f"simtime range: ({start}, {end}] retriving {data.shape[0]:,} rows took {it.default_timer() - ts:2.2f} seconds for {(s1 + s2):,.2f}MB (sizeoff data+glb)")
                start = end 
                end = start + time_chunk_size
                yield glb, data
    ...
    def _create_hdf(self, count_p: DpmmCount, builder: CellEntropyValueErrorBuilder, with_rsd: bool):
        ...
        for glb, data in self.time_chunked_stream(count_p, time_chunk_size, time_interval, builder):
            ...
            self.hdf_cell_entropy_measure.write_frame(
                group=self.hdf_cell_entropy_measure.group, 
                frame=aggregate_all
            )
            del aggregate_all
            aggregate_all = None
...
class CellCountError:
    ...
    def get_or_create(
        ...
        build_new = False
        if os.path.exists(hdf_path):
            for g in [obj.g_error_cell_all, obj.g_error_cell_rsd_all, obj.g_error_cell_rsd_local]:
                h = BaseHdfProvider(hdf_path, group=g)
                if not h.contains_group(g):
                    build_new = True
                    break
    ...
    def hdf_cell_measure(self) -> BaseHdfProvider:
        if self._hdf_cell_measure is None:
            self._hdf_cell_measure = BaseHdfProvider(self.hdf_path, group=self.g_error_cell_all)
        return self._hdf_cell_measure
    
    @property
    def hdf_cell_measure_rsd(self) -> BaseHdfProvider:
        if self._hdf_cell_measure_rsd is None:
            self._hdf_cell_measure_rsd = BaseHdfProvider(self.hdf_path, group=self.g_error_cell_rsd_all)
    ...
    def hdf_cell_measure_rsd_local(self) -> BaseHdfProvider:
        if self._hdf_cell_measure_rsd_local is None:
            self._hdf_cell_measure_rsd_local = BaseHdfProvider(self.hdf_path, group=self.g_error_cell_rsd_local)
    ...
    def load_data(self, count_p: DpmmCount, time_slice: slice, builder:CellCountErrorBuilder) -> pd.DataFrame:
        _i = pd.IndexSlice
        # all (time, x, y, id) based count, err, squerr cell values
        # without ground truth (see slice last slice `1:`)
        # The measurements are summed over all nodes (this will drop the id index )
    ...
    def time_chunked_stream(self, count_p:DpmmCount, time_chunk_size, time_interval, builder:CellCountErrorBuilder):
        if time_chunk_size is None:
            time_slice = slice() # empty slice, no chunking load all data (can cause OOM!)
            glb = self.load_glb(count_p, start, end, builder)
            data = self.load_data(start, end, builder)
    ...
    def _aggregate_time_chunk(self, glb:pd.DataFrame, nodes:pd.DataFrame, builder: CellCountErrorBuilder) -> pd.DataFrame:
        glb = glb.droplevel("ID")
        glb.columns = ["glb_count"]
    ...
    def _aggregate_time_chunk_rsd(self, hdf:BaseHdfProvider, mask, glb:pd.DataFrame, data:pd.DataFrame, rsd:int, builder: CellCountErrorBuilder):
        # select subset of cell that belong to provided rsd. 
        idx_ = data[mask].index.droplevel("ID").unique().to_frame().reset_index(drop=True)
        idx_["ID"] = 0
        idx_ = pd.MultiIndex.from_frame(idx_).sort_values()
...
class MapCountError:
    ...
    def get_or_create(
        ...
        build_new = False
        if os.path.exists(hdf_path):
            for g in [obj.g_error_count_all, obj.g_error_count_rsd_all, obj.g_error_count_rsd_local]:
                h = BaseHdfProvider(hdf_path, group=g)
                if not h.contains_group(g):
                    build_new = True
                    break
    ...
    def is_initialized(self):
        if os.path.exists(self.hdf_path):
            for g in [self.g_error_count_all, self.g_error_count_rsd_all, self.g_error_count_rsd_local]:
    ...
    def hdf_map_measure_rsd(self) -> BaseHdfProvider:
        if self._hdf_map_measure_rsd is None:
            self._hdf_map_measure_rsd = BaseHdfProvider(
                self.hdf_path, group=self.g_error_count_rsd_all, allow_lazy_loading=False
            )
    ...
    def hdf_map_measure_rsd_local(self) -> BaseHdfProvider:
        if self._hdf_map_measure_rsd_local is None:
            self._hdf_map_measure_rsd_local = BaseHdfProvider(
                self.hdf_path, group=self.g_error_count_rsd_local, allow_lazy_loading=False
            )
        return self._hdf_map_measure_rsd_local

    def _pull_ground_truth_data(self, glb_pos, rsd_p:RsdAssociationProvider|None = None) -> Tuple[pd.DataFrame, pd.DataFrame]:
        glb_all = glb_pos.groupby(["simtime"])[["x"]].count().set_axis(["count"], axis=1)
        glb_rsd = glb_pos
        if "servingEnb" not in glb_rsd:
            if rsd_p is None:
                raise ValueError("Rsd association missing in postion data. Either provide an RsdAssociationProvider or update the global position dataframe with a servingEnb column")
            else: 
                glb_rsd = rsd_p.merge_rsd_id_on_host_time_interval(glb_pos, "node_id", "simtime", append_interval=False)
        glb_rsd = glb_rsd.groupby(["servingEnb", "simtime"])[["x"]].count().set_axis(["count"], axis=1)
    ...
    def _append_rsd_and_write(self, hdf: BaseHdfProvider, data: pd.DataFrame, rsd:int):
        ...
        if any(_mask_nan):
            ...
            data = data[~_mask_nan]
        hdf.write_frame(group=hdf.group, frame=data, index=True, index_data_columns=True)
    ...
    def time_chunked_stream(self, map_p: DpmmProvider, chunk_size, time_interval):
        if chunk_size is None:
            _i = pd.IndexSlice
            yield map_p[_i[:, :, :, 1:], _i["count", "rsd_id", "owner_rsd_id"]]  # all but ground truth
        else:
            start = time_interval[0]
            end = start + chunk_size
            while(start < time_interval[1]):
                ts = it.default_timer()
                n = map_p.select(where=f"simtime >= {start} and simtime < {end}", columns=["count", "rsd_id", "owner_rsd_id"])
                size = sys.getsizeof(n)/1e6
                print(f"simtime range: ({start}, {end}] retriving {n.shape[0]:,} rows took {it.default_timer() - ts:2.2f} seconds for {size:,.2f}MB (sizeoff)")
                start = end 
                end = start + chunk_size
                yield n
    ...
    def _aggregate_time_chunk(self, glb:pd.DataFrame, glb_rsd:pd.DataFrame, data:pd.DataFrame, rsd_list:List[int]):
        ...
        for rsd in rsd_list:
            ...
            data_rsd = self._aggregate_group(glb_rsd.loc[rsd].copy(), node_data=data[mask_rsd]["count"].copy())
            ...
            data_rsd_local = self._aggregate_group(glb_rsd.loc[rsd].copy(), node_data=data[mask_rsd_local]["count"].copy())
            self._append_rsd_and_write(self.hdf_map_measure_rsd_local, data=data_rsd_local, rsd=rsd)
            del data_rsd_local
