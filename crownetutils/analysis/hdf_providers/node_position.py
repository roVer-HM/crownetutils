...
from crownetutils.analysis.hdf.provider import BaseHdfProvider, HdfGroupFactory
...
class RsdPositionHdf(BaseHdfProvider):
    @classmethod
    def get(cls, hdf_path: str, sql: CrownetSql, **kwargs) -> RsdPositionHdf:
        obj = cls(hdf_path=hdf_path, group="rsd_pos", allow_lazy_loading=True)
        if len(kwargs) > 0:
            obj.add_group_factory(
                HdfGroupFactory(
                    obj.group, partial(sql.get_resource_sharing_domains, **kwargs)
                )
            )
        else:
            obj.add_group_factory(
                HdfGroupFactory(obj.group, sql.get_resource_sharing_domains)
            )
        return obj

    @classmethod
    def from_sim(cls, sim: Simulation, **kwargs) -> RsdPositionHdf:
        return cls.get(sim.path("position.h5"), sim.sql, **kwargs)
