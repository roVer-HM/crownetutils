...
class VadereProjectTests(unittest.TestCase):
    def test_project_name(self):
        project = VadereProject("testData/s2ucre")
        self.assertEqual(project.project_name, "s2ucre_scenarios")

    def test_wrong_project_dir(self):
        self.assertRaises(FileNotFoundError, VadereProject, "testData/s2uSSScre")

    def test_no_output_dir(self):
        shutil.rmtree(
            os.path.join("testData/s2ucreInvalid", "output"), ignore_errors=True
        )
        self.assertFalse(
            os.path.exists(os.path.join("testData/s2ucreInvalid", "output"))
        )
        p = VadereProject("testData/s2ucreInvalid")
        self.assertTrue(
            os.path.exists(os.path.join("testData/s2ucreInvalid", "output"))
        )
    ...
    def test_scenaio_files(self):
        ...
        self.assertEqual(
            [
                ...
                "testData/s2ucre/scenarios/empty.scenario",
            ],
