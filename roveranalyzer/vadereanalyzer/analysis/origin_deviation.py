...
class OriginDeviation:
    ...
    def get_origin_deviation_data(self):
        ...
        if self.name == "":
            self.name = (
                out_0_origin.output_dir_name + "---" + out_offset_origin.output_dir_name
            )
        ...
        df_max_diff = df.loc[
            df.groupby("pedestrianId")["diff"].idxmax(),
        ]
    ...
    def get_origin_deviation_result(self):
        ...
        ped_id_warn = df_max_diff[
            (df_max_diff["diff"] > self.max_diff_ok)
            & (df_max_diff["diff"] <= self.max_diff_warn)
        ].index.values
        ped_id_err = df_max_diff[
            (df_max_diff["diff"] > self.max_diff_warn)
        ].index.values
        ped_id_ok_ = df_max_diff[(df_max_diff["diff"] <= self.max_diff_ok)].index.values

        ret = {
            "err_count": len(ped_id_err),
            "warn_count": len(ped_id_warn),
            "ok_count": len(ped_id_ok_),
            "err_dict": df_max_diff.loc[ped_id_err].to_dict("index"),
            "warn_dict": df_max_diff.loc[ped_id_warn].to_dict("index"),
            "stats_at_max_diff": df_max_diff["diff"].describe(),
            "stats_at_start": df_start_diff["diff"].describe(),
            "output_name": self.name,
            "df_max_diff": df_max_diff,
            "df_start_diff": df_start_diff,
        }
