...
class VadereProject:
    ...
    def __init__(self, project_dir, expect_all_outputs: bool = True):
        ...
        if os.path.exists(project_dir):
            ...
            raise FileNotFoundError(
                "Directory at {} does not exist.".format(project_dir)
            )
        ...
        if os.path.exists(os.path.join(self.project_path, "output")):
            ...
            print(
                "Warn: project {} has no output folder, an empty folder will be created.".format(
                    self.project_name
                )
            )
    ...
    def _load_output_directories(self, expect_all_outputs):
        ...
        out_dirs = [
            dir
            for dir in all_dirs
            if dir != "corrupt" and dir != "legacy" and not dir.startswith(".")
        ]
