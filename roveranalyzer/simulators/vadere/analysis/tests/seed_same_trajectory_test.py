...
from simulators.vadere.analysis.seed_same_trajectory import SameSeedTrajectory


class SameSeedTrajectoryTest(unittest.TestCase):
    def test_find_same_scenarios(self):
        test = SameSeedTrajectory("../../../simulators/vadere/tests/testData/s2ucre")
    ...
    def test_trajectory_seed(self):
        ...
        self.assertEqual(
            True, bridge_coordinates_kai_navigation_random_pos["hash_is_equal"]
        )
        self.assertEqual(
            2, len(bridge_coordinates_kai_navigation_random_pos["scenario_outputs"])
        )
        ...
        self.assertEqual(
            True, bridge_coordinates_kai_origin_0_navigation["hash_is_equal"]
        )
        self.assertEqual(
            2, len(bridge_coordinates_kai_origin_0_navigation["scenario_outputs"])
        )

        bridge_coordinates_kai_origin_0_navigation_random_pos = out[5]
        self.assertEqual(
            True, bridge_coordinates_kai_origin_0_navigation_random_pos["hash_is_equal"]
        )
        self.assertEqual(
            2,
            len(
                bridge_coordinates_kai_origin_0_navigation_random_pos[
                    "scenario_outputs"
                ]
            ),
        )
        ...
        self.assertEqual(
            True, bridge_coordinates_kai_origin_0_random_pos["hash_is_equal"]
        )
        self.assertEqual(
            2, len(bridge_coordinates_kai_origin_0_random_pos["scenario_outputs"])
        )
